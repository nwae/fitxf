...
class Voice2Array:
    ...
    def play_voice_stream(
        ...
        sample_rate, n_channels, x = self.read_audio(file_path_or_base64_str=file_path_or_base64_str)
        sample_width = self.get_sample_width(x=x)
        if n_channels is not None:
            assert channels == n_channels, \
                'Num channels from file is ' + str(n_channels) + ', but given ' + str(channels)
        # we need to normalize audio data to range [-1, +1] before play back
        # x = self.normalize_audio_data(x=x)
